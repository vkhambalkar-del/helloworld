...
import { UserListContainer, buildColDefs } from './UserListContainer';
...
describe('buildColDefs', () => {
  ...
  it('currency valueFormatter formats numeric values', () => {
    const cols = buildColDefs(sampleManifest);
    const salaryCol = cols.find((c) => c.field === 'salary')!;
    const formatter = salaryCol.valueFormatter as (params: { value: number | null }) => string;
    expect(formatter({ value: 95000 })).toBe('$95,000');
  });

  it('currency valueFormatter returns empty string for null', () => {
    const cols = buildColDefs(sampleManifest);
    const salaryCol = cols.find((c) => c.field === 'salary')!;
    const formatter = salaryCol.valueFormatter as (params: { value: number | null }) => string;
  ...
  it('boolean cellRenderer returns Yes for true', () => {
    const cols = buildColDefs(sampleManifest);
    const activeCol = cols.find((c) => c.field === 'active')!;
    const renderer = activeCol.cellRenderer as (params: { value: boolean }) => string;
    expect(renderer({ value: true })).toBe('Yes');
  });

  it('boolean cellRenderer returns No for false', () => {
    const cols = buildColDefs(sampleManifest);
    const activeCol = cols.find((c) => c.field === 'active')!;
    const renderer = activeCol.cellRenderer as (params: { value: boolean }) => string;
    expect(renderer({ value: false })).toBe('No');
  });
...
// ── UserListContainer component tests ──────────────────────────────────

// Mock DataGrid to capture what UserList (child) receives
...
describe('UserListContainer', () => {
  ...
  it('renders page title "Users" and DataGrid when loaded', () => {
  ...
  it('dispatches fetchUsersManifest on mount', () => {
    const store = createTestStore();
    render(
      <Provider store={store}>
        <UserListContainer />
  ...
  it('dispatches fetchUsers when columns are available', () => {
    const store = createTestStore({
      columns: sampleManifest,
      users: [],
    });
  ...
  it('does not dispatch fetchUsers when columns are empty', () => {
